...
> Bootstrap design background. For the current Fedora 44/45 x86_64 matrix and
> executable release procedure, use [copr-project.md](copr-project.md) and
> [operations.md](operations.md). Future goals below are not support promises.

## Summary

Build and maintain a small Fedora COPR named `dtutila/hyprland` that
...
- Build for `x86_64`; retain architecture-neutral sources for future `aarch64` validation.
...
- **Architectures:** `x86_64` currently; `aarch64` is a future expansion.
- **Publication:** manual repository generation after release-set tests.
